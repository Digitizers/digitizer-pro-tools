...
Create `modules/agent-log/class-dpt-al-admin.php` with `register_menu( $parent_slug )` adding a submenu titled `__( 'Agent Log', 'digitizer-pro-tools' )` under `$parent_slug` with capability `manage_options` and slug `dpt-agent-log`; a `render_page()` that reads `$_GET['channel']` and `$_GET['object_type']` through `sanitize_key( wp_unslash( ... ) )`, passes them to `DPT_AL_Store::query()`, and prints a `wp-list-table widefat striped` table of the columns, escaping every cell with `esc_html()` and rendering `logged_at` through `date_i18n( get_option( 'date_format' ) . ' ' . get_option( 'time_format' ), strtotime( $row->logged_at . ' UTC' ) )`; and an `admin_post_dpt_al_clear` handler that calls `check_admin_referer( 'dpt_al_clear' )`, refuses without `current_user_can( 'manage_options' )` via `wp_die( esc_html__( 'You are not allowed to do that.', 'digitizer-pro-tools' ) )`, truncates the table, and redirects back. Follow `modules/update-policy/class-dpt-up-admin.php` for the exact shape of the nonce, capability and redirect handling.

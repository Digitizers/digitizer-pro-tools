...
* Optionally removes the unused default themes, always keeping the newest as the fallback WordPress needs if the active theme ever breaks. The active theme, and any theme another theme depends on, are never removed
...
* Onboarding can remove the default themes a finished site no longer needs. It always keeps the newest one, because that is what WordPress falls back to if the active theme ever breaks, and it never removes the active theme or a theme another theme depends on
